...
const realTimeQueueController = {
    ...
    async create(req, res) {
        try {
            ...
            if (!commentID) {
                return res.status(400).json(Response(400, 'Емхана нің IDы берылмеген!'));
            }
            ...
            return res.status(500).json(Response(500, 'Кезек жасалудан қате щықты!'));
        }
    },
    ...
    async getByCode(req, res) {
        try {
            ...
            if (!realTimeQueue) {
                return res.status(404).json(Response(404, 'Таңба жоқ!'));
            }
            return res.status(200).json(realTimeQueue);
        } catch (err) {
            return res.status(500).json(Response(500, 'Кезек жасалмаған!'));
        }
    },
    ...
    async next(req,res){
        try{
            ...
            if(queue_max_index <0 || realTimeQueue.queue_index >= queue_max_index){
                return res.status(404).json(Response(404, 'Келесы кезең жоқ!'));
            }
