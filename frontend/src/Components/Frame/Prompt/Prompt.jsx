...
export default function Prompt({ inputClassName, message, OnClick, OnClosed }) {
  ...
  return (
    <div className=" flex w-full h-full justify-center items-center z-30 text-white">
      <div
        className=" absolute top-0 left-0 w-full h-full bg-black opacity-20 z-22"
        onClick={() => OnClosed(inputValue)}></div>
      <div className="flex flex-row justify-center items-center bg-secondary-blue z-26">
        <label htmlFor="input">{message}</label>
        <input
          className={
            inputClassName ??
            "pl-3 pr-3 pt-1 pb-1 bg-primary-blue rounded-3xl focus:outline-none w-36"
          }
          type="text"
          value={inputValue}
